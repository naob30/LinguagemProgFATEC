...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $_SESSION['mensagem'] = "EDITORA CADASTRADA COM SUCESSO!";
    header('Location: addEditora.php');
    exit();
}
?>
...
<body>
    <header>
        <h3 class="text-uppercase text-center" style="color: white; font-size: 38px">Cadastrar EDITORAS</h3>
    </header>
    ...
    <div class="d-flex justify-content-center banner">
        <div class="card d-flex justify-content-center">
            <div class="card-header font-monospace text-uppercase">
                Cadastrar EDITORAS da biblioteca
            </div>
            <div class="card-body">
                <form action="" method="POST">
                    <div class="input-group flex-nowrap">
                        <span class="input-group-text" id="addon-wrapping"><i class="bi bi-postcard"></i></span>
                        <input type="text" class="form-control font-monospace" placeholder="Digite a editora..." aria-label="Username" aria-describedby="addon-wrapping" name="nomeEditora" required>
                    </div>

                    <input type="submit" value="CADASTRAR EDITORA" class="btn btn-outline-primary">
                </form>
            </div>
        </div>
    </div>
